...
export class ClientSvgError extends Error {
  constructor(message) {
    super(message)
    this.name = 'ClientSvgError'
  }
}
...
function decorate(root, metadata, presentation, engine) {
  ...
  if ((presentation.padding || presentation.background || presentation.frame) && bounds === null) {
    throw new ClientSvgError('Renderer output has no usable SVG dimensions for presentation settings.')
  }
  if (bounds !== null) {
...
export function sanitizeAndDecorateSvg(source, metadata, presentation, engine) {
  if (source.length > MAX_SVG_LENGTH) throw new ClientSvgError('Rendered SVG is too large.')
  let root = null
  const stack = []
  let skippedDepth = 0
  let parseError = null
  ...
  const appendText = value => {
    ...
    const parent = stack.at(-1)
    if (parent) {
      if (localName(parent) === 'style' && unsafeCss(value)) throw new ClientSvgError('Renderer output contains unsafe CSS.')
  ...
  try {
    ...
    if (error instanceof ClientSvgError) throw error
    parseError = error
  }
  if (parseError !== null || root === null || localName(root) !== 'svg') {
    throw new ClientSvgError('Renderer returned invalid SVG.')
  }
