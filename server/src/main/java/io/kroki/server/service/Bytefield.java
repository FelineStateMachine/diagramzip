...
public class Bytefield implements DiagramService {
  ...
  public Bytefield(Vertx vertx, JsonObject config, Commander commander) {
    this.vertx = vertx;
    this.binPath = config.getString("KROKI_BYTEFIELD_BIN_PATH", "bytefield-svg");
  ...
  public Future<Buffer> convert(String sourceDecoded, String serviceName, FileFormat fileFormat, JsonObject options, RenderCancellation cancellation) {
    return vertx.executeBlocking(() -> {
      byte[] result = bytefield(cancellation, sourceDecoded.getBytes());
      return Buffer.buffer(result);
    });
  }

  private byte[] bytefield(RenderCancellation cancellation, byte[] source) throws IOException, InterruptedException, IllegalStateException {
