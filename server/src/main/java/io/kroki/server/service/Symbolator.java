...
public class Symbolator implements DiagramService {

  private static final List<FileFormat> SUPPORTED_FORMATS = Arrays.asList(FileFormat.SVG, FileFormat.PNG);
  private final SourceDecoder sourceDecoder;
  private final Vertx vertx;
  private final SymbolatorCommand command;

  public Symbolator(Vertx vertx, JsonObject config) {
    this.vertx = vertx;
    this.command = new SymbolatorCommand(config);
