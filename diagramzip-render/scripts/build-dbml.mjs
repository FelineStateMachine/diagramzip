...
await build({
  entryPoints: ['src/vendor/dbml-entry.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  minify: true,
  outfile: 'src/vendor/dbml-backend.js',
  plugins: [{
    name: 'dbml-dot-only',
    setup(plugin) {
...
const { dbmlToDot } = await import(new URL(`../src/vendor/dbml-backend.js?build=${Date.now()}`, import.meta.url))
