...
| `X-Tiny-Relay` | The relay origin. |
| `X-Tiny-Signature` | `sha256=<hex>`, the HMAC-SHA256 of the raw request body. |
...
The operator stores the secret as a Workers secret named `TINY_TRANSFORM_SECRET`:

```sh
wrangler secret put TINY_TRANSFORM_SECRET --config services/shell/wrangler.jsonc
```
