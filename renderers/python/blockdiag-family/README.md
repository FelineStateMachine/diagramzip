...
The six engines share one Worker because they form one compatible upstream
dependency family. The four source trees provide the same parser, builder, SVG
drawer, font, and node-renderer model. They retain six hostnames, so the hostname
remains the only engine selector. The shared deployment avoids six copies of
Pillow, the font, and the registration table. One request cannot select another
engine.
...
domain emulation resolves every configured hostname as the first route. Use the
CPython six-engine example suite locally. Smoke all six real hostnames after
deployment. Do not treat a local `Host` header as proof of per-host dispatch.
...
The request protocol enforces a 1 MiB body limit and a 512 KiB source limit. It
allows at most 64 options and 4 KiB per option. Padding ranges from 0 through
256. The sanitized SVG limit is 4 MiB. Only SVG output is exposed. Remote URLs
and filesystem images are rejected before they can be fetched. Returned SVG is parsed with
...
`no-transparency`, intentionally have no effect on SVG. The common protocol
validates other upstream command-line options but does not pass them to a CLI.
They remain documented loss until a direct API mapping is added. The Worker
uses the bundled DejaVu Serif font path from the Docker reference. This path
provides deterministic text metrics. Font fallback can differ on a larger host.
...
are synchronous with no `await` boundary. Requests cannot interleave while one
isolate changes those registries. Cloudflare can run multiple isolates. Each
isolate has its own Python runtime and registries. Keep rendering synchronous
until the registries become request-local.
