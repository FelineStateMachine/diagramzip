...
- `plantuml.js`: `@plantuml/core@1.2026.6`, SHA-256
  ...
  `vendor-licenses/plantuml-core-NOTICE`.
- `viz-global.cjs`: the official package's `viz-global.js` in its native UMD
  CommonJS form, patched only to replace
  Emscripten's dynamic `WebAssembly.instantiate` with a precompiled module
  bridge and its Node-only `__filename` URL fallback with a fixed inert URL.
...
rendering in real workerd when a small DOM adapter is present and the Viz
module is provided as Wrangler's precompiled Wasm import. The C4 adapter
lowers the bounded macro surface to ordinary PlantUML primitives and rejects
unsupported macros explicitly.

Use `wrangler.plantuml-family.local.jsonc` for local multi-host workerd tests.
Wrangler's local route emulation rewrites requests to the first configured
custom domain when the production config is used, which would otherwise make
the C4 hostname exercise the PlantUML descriptor.
