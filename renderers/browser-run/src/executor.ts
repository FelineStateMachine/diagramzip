  ...
  private async pageFor(engine: RenderRequest['engine']): Promise<Page> {
    if (!this.browser?.isConnected()) {
      this.browser = await puppeteer.launch(this.endpoint, {
        keep_alive: 60_000,
        guardrails: {
          allowedDomains: ['bpmn.render.diagram.zip', 'mermaid.render.diagram.zip', 'excalidraw.render.diagram.zip', 'diagramsnet.render.diagram.zip', 'tikz.render.diagram.zip'],
        },
      })
      this.page = undefined
