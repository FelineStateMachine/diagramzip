...
done < <(find apps/docs services renderers/client renderers/edge -name package-lock.json -not -path '*/node_modules/*' -print | sort)
...
DEPLOYMENT_COUNT=$((EDGE_CONFIG_COUNT + ${#PYTHON_DIRECTORIES[@]} + ${#CLIENT_DIRECTORIES[@]} + 4))
...
printf '  Discovered: %s edge + %s Python + %s client + catalog/API/shell/docs = %s deployments\n' \
